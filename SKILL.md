...
把科研图、示意图、幻灯片截图或 UI 截图重建成可编辑图形资产的 skill。The workflow focuses on editable Office Shapes first, adds a reusable scene manifest, and can export SVG for editable scientific figures outside Office. It must protect image quality with lossless/high-resolution assets, and it must be explicit about the tradeoff between high-fidelity raster preservation and true editability.
...
Convert a reference image into editable reconstruction artifacts. The default Office path produces a complete, runnable VBA macro or editable PPTX that rebuilds the visual using editable Shapes, TextBoxes, Lines, Freeforms, fills, gradients, transparency, shadows, and ZOrder. When the user asks for SVG or editable scientific figures, also produce a `scene_manifest.json` and an editable `.svg` from the same element plan. When the image contains complex, distinctive graphic elements that would become noisy or low-value if rebuilt as hundreds of shapes, preserve those elements as cropped local image assets and reconstruct the surrounding structure as editable objects. The core loop is: image -> lossless/high-res asset preparation -> environment detection -> visual decomposition -> editability target selection -> editable-vs-preserved element plan -> scene manifest -> Office coordinate model and/or SVG canvas model -> VBA/SVG/cropped assets -> run or prepare the macro/deck -> self-check -> iterative correction.

Prefer editable Office Shapes or SVG vector elements for all structure, labels, arrows, boxes, chart scaffolds, legends, axes, readable text, and callouts. Preserve complex graphic elements only as local cropped assets when that improves fidelity and keeps the output maintainable. Do not embed the full source image or a few large panel screenshots as a shortcut when the user asked for "可编辑 PPT" unless those image layers are explicitly labeled as preserved, non-editable regions and the editable overlays are still reconstructed. A PNG/JPEG/PDF preview is never the final deliverable for this skill; it is only allowed as a diagnostic screenshot after an editable reconstruction has been produced or attempted.

## Image Quality Contract

Never degrade the user's source image quality. Before reconstruction, create lossless/high-resolution working assets when local file access is available:

```bash
python scripts/prepare_highres_assets.py path/to/source.png path/to/output/assets/highres --scale 2
```

Rules:

- Keep the original source untouched.
- Convert JPEG or pasted images to `source_lossless.png` before cropping or embedding.
- Generate a `source_highres_2x.png` working image by default. Use 4x only when the original is small, blurry, or the user explicitly asks for ultra-high resolution.
- Do all raster crops from the lossless/high-res working image, not from a compressed preview or screenshot.
- Save all preserved assets as PNG, not JPEG. Use no lossy compression.
- When generating PPTX, embed original-resolution or high-res PNG assets. Do not downsample images unless the user explicitly asks for a smaller file.
- If exporting a rendered preview after editing, render it at 2x or higher and save PNG.
- Final reports must state the source size, high-res working size, and whether any raster assets were preserved.
...
- A `scene_manifest.json` when the user asks for SVG, multi-format output, reusable reconstruction data, or editable scientific figures outside Office.
- A complete VBA macro file or code block that reconstructs the image with editable Office Shapes.
- An editable `.svg` when SVG is requested or when the user says the goal is "科研图可修改", "vector", "矢量", "网页可编辑", or "illustrator/inkscape 可改".
...
- A short run report that says whether the macro was executed, whether the deck opened, and where the generated `scene_manifest.json` / `.svg` / `.bas` / `.pptx` / `.pptm` files are located.
...
Choose outputs by user intent:

- PowerPoint / Office / WPS / PPT: produce `.bas` or `.vba`, attempt `.pptx` or `.pptm`, and create preserved crop assets only when needed.
- SVG / vector / Illustrator / Inkscape / 科研图可修改: produce `scene_manifest.json` and `.svg`; also produce VBA/PPT when Office editing is requested.
- "All formats", "多格式", or ambiguous commercial/reuse request: produce `scene_manifest.json`, `.svg`, `.bas`, and the best available deck attempt.
- Pure analysis request: explain likely reconstruction strategy and limitations; do not fabricate output artifacts.

SVG is an editable vector/raster-hybrid document. Rectangles, text, arrows, axes, simple icons, and callouts become SVG elements. Preserved microscopy/photos/3D renders/logos remain embedded or linked raster image elements inside the SVG and must be reported as non-vector preserved assets.

## Editability Modes

Choose a mode before generating outputs and state it in the report.

### Mode A: High-Fidelity Hybrid

Use this when the user prioritizes "不要出错", "不要重叠", "保真", "先稳定", or when the visual contains microscopy, heatmaps, dense dendrograms, photos, or complex schematics. Dense regions may be preserved as high-res raster crops. However, reconstruct visible labels, panel letters, legends, axis titles, arrows, callouts, and captions as editable objects whenever practical.

For commercial or client-facing delivery, Mode A is the default unless the user explicitly accepts manual-quality reconstruction time. The first deliverable must be visually correct and complete. A second editable annotation layer may be included for labels/arrows that are unsafe to integrate directly without damaging the raster figure.

Do not preserve an entire panel as one image if it contains easily editable outer text/labels. At minimum, split each panel into:

- preserved core raster region: microscope image, heatmap body, complex illustration, dense plot area;
- editable overlay region: titles, panel letters, labels, legends, arrows, axes, caption, simple boxes.

### Mode B: High-Editability Reconstruction

Use this when the user asks "里面都能改", "不要只是几张图", "尽量全部可编辑", "PPT 里文字箭头都能编辑", or complains that the output is only images. In this mode:

- Extract every legible text label into editable TextBoxes.
- Rebuild arrows, leader lines, brackets, panel borders, legends, color bars, simple icons, and simple shapes as editable objects.
- Preserve only truly complex raster cores such as microscopy image pixels, dense heatmap cells, photo-like renderings, or highly detailed illustrations.
- If a panel must keep a raster core, crop tightly around the core and overlay editable text/markers around it instead of preserving the whole panel.
- Increase object count rather than using a few big images, unless doing so would introduce obvious wrong geometry.
- Never duplicate text that is still visible inside a preserved raster crop. If a label/title/caption is rebuilt as editable text, the raster crop must exclude that original text area or the original text must be covered by a clean background mask first.
- Do not place editable labels over preserved image labels unless the editable label is intentionally replacing hidden/covered text. Duplicate text is a hard layout failure.
- Do not use broad white/black rectangles to hide original text when the rectangle also covers scientific image content, heatmaps, microscopy, schematic bodies, or connectors. Destructive masking that erases source content is a delivery failure.
- If the original text is embedded inside a complex image region and cannot be removed cleanly, keep the fidelity-first raster intact and put the reconstructed text/arrow on a separate editable annotation slide/layer. Report this explicitly instead of pretending the integrated slide is fully editable.

When the user asks for both "no errors/no overlap" and "fully editable", explain that this is a tradeoff. Default to Mode B for text/arrows/labels and Mode A only for complex raster cores.

### Mode C: Commercial Delivery Package

Use this when the user says the result is for GitHub release, customers, resale, public demo, "交付", "售卖", "给别人使用", or when previous fully-integrated attempts caused overlap, missing images, or wrong text.

Mode C produces a package instead of a single overpromised slide:

- Slide/page 1: fidelity-first deliverable with complete high-resolution, lossless image layers and no destructive masking.
- Slide/page 2: editable working deliverable. Keep complete panel geometry and complete image layers wherever possible, locally mask only the original text/arrow regions that will be replaced, then rebuild text, arrows, labels, legends, and callouts as editable objects.
- SVG: separate high-resolution image layers plus editable external/caption text where safe.
- `scene_manifest.json`: lists preserved raster elements, editable annotation elements, and any unsafe-to-integrate text regions.
- Report: clearly states what is editable, what remains raster, and why.

Mode C is preferable to a visually broken "fully editable" slide. Never ship transparent-reference slides, half-cropped image cores, or integrated overlays that visibly erase image content, duplicate OCR text, or misalign labels just to satisfy editability metrics.

For flattened scientific figures, the preferred commercial pattern is the v8 pattern:

1. Create a 2x lossless working image.
2. Crop complete panel PNGs from the working image.
3. Duplicate those complete panel PNGs into masked panel PNGs.
4. Mask only local text/arrow regions that will be replaced by editable objects.
5. Place the masked complete panels at the original coordinates.
6. Add editable text, arrows, labels, legends, and captions on top.
7. Also keep a fidelity slide/page with unmasked complete panels for comparison.
...
Hard preservation triggers (hit any one → the element MAY be preserved as a raster crop, but only for the smallest useful core region):
...
- A faithful editable reconstruction would require more than ~30 Shapes for that single element in Mode A, or more than ~80 Shapes in Mode B.
...
If none of the triggers fire, treat the element as editable. If a trigger fires but the user demanded "all editable", explain the fidelity tradeoff first, then vectorize/approximate simple parts and preserve only the irreducible raster core.
...
- Preserve only the smallest useful crop around each complex element; do not insert the entire source image or entire panel just to preserve a small object.
- If preserving a whole panel, the report must justify why its internal labels/arrows cannot be reconstructed separately. This should be rare.
- If preserving a whole panel, do not add duplicate editable text inside that panel. Add editable objects only outside the panel crop or over areas that have been intentionally masked.
- If a whole panel is preserved for fidelity in a commercial package, also provide a masked complete-panel working version before adding internal editable text/arrows. Do not use a low-opacity reference layer as the final editable slide.
...
- If the user requests "all editable", do not use large preserved raster panels. Use small raster cores plus editable overlays, and clearly list anything that remains non-editable.
...
Output the working manifest as a Markdown table or JSON list with these fields per element:
...
When SVG or multi-format output is requested, normalize this working manifest into `scene_manifest.json` before generating format-specific code. Load `references/scene-manifest.md` for the schema. The scene manifest is the source of truth for `.svg`, `.bas`, and any fallback deck so the formats do not drift.
...
When SVG is requested, generate `scene_manifest.json` first, then run:

```bash
python scripts/manifest_to_svg.py path/to/scene_manifest.json -o path/to/reconstruct.svg
```

SVG generation rules:

- Use the source image pixel coordinate system for the SVG `viewBox` unless the user requests a different canvas.
- Map editable rectangles, rounded rectangles, ellipses, lines, arrows, polylines, polygons, and text to SVG elements.
- Map preserved raster crops to `<image>` elements with `preserveAspectRatio="xMidYMid meet"`.
- Keep element ids stable and aligned with the manifest.
- Report SVG limitations plainly: Office-only effects, text wrapping, gradients, shadows, and font substitution may differ.
- Do not claim preserved raster crops are vectorized; they are editable as positioned image elements only.
...
For SVG or multi-format targets, after generating the scene manifest:

1. Save `scene_manifest.json`.
2. Save preserved crop assets if hybrid mode is used.
3. Run `scripts/manifest_to_svg.py`.
4. Open or inspect the SVG when local tooling is available, or at least verify it is well-formed XML and contains the expected element ids.
5. Report the `.svg` path separately from Office artifacts.
...
4. Scene manifest check: when `scene_manifest.json` is generated, verify required top-level fields, element ids, z-order, asset paths, and format outputs.
5. SVG check: when SVG is generated, verify the file exists, is parseable XML, contains expected element ids, and distinguishes vector elements from preserved raster `<image>` elements.
...
7. Materialization check: verify that the macro was saved, that preserved assets were saved when needed, that the available office runtime was detected, that the appropriate PowerPoint/WPS path was attempted, and that the output deck is open or a concrete blocker is reported.
8. Visual check (render-verify 强制闭环): 不允许停留在"人工目测"。这是保证还原度的关键环节，必须按下面步骤反复迭代直到达标或触发硬性上限。
...
- Artifact paths for `scene_manifest.json` and `.svg` when generated.
...
- `scripts/manifest_to_svg.py`: convert `scene_manifest.json` into editable SVG.
- `references/scene-manifest.md`: shared reconstruction schema for VBA/SVG/multi-format outputs.
